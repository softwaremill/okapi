...
class MicrometerOutboxListenerTest : FunSpec({
    ...
    test("Delivered event increments delivered counter") {
        ...
        registry.counter("okapi.entries.retried").count() shouldBe 0.0
        registry.counter("okapi.entries.failed").count() shouldBe 0.0
    }

    test("Retried event increments retried counter") {
        val registry = SimpleMeterRegistry()
        val listener = MicrometerOutboxListener(registry)
        val entry = stubEntry().retry(Instant.EPOCH, "timeout")

        listener.onEntryProcessed(OutboxProcessingEvent.Retried(entry, Duration.ofMillis(10), "timeout"))

        registry.counter("okapi.entries.retried").count() shouldBe 1.0
    }
